...
registerUser : catchAsyncErrors(async (req, res, next) => {
    ...
    if (!name) {
        return next(new ErrorHandler('Tên không được để trống', 400))
    }
    if (!email) {
        return next(new ErrorHandler('Email không được để trống', 400))
    }
    if (!password) {
        return next(new ErrorHandler('Mật khẩu không được để trống', 400))
    }
    if (!avatar) {
